...
class FinancialSummaryView(APIView):
    def get(self, request, *args, **kwargs):
        ...
        all_time_summary = Transaction.objects.filter(user=user)\
            ...
            .annotate(income=Sum('amount', filter=Q(transaction_type='INC')), expense=Sum('amount', filter=Q(transaction_type='EXP'))).order_by('month')
        this_month_summary = Transaction.objects.filter(user=user, transaction_date__year=today.year, transaction_date__month=today.month)\
